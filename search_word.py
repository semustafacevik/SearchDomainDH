...
class SearchWord(Resource):
    ...
    # baidu
    def do_search_baidu(self, word):
        print('Searching Baidu...')
        self.word = word
        self.total_results_baidu = ''
        ...
        request = (grequests.get(url, headers=headers, timeout=1) for url in urls)
        responses = grequests.imap(request, size=5)
        for response in responses:
            self.total_results_baidu += response.content.decode('UTF-8')
        result['result_baidu'] = self.total_results_baidu
        print('OK - Baidu!')
    ################################

    # bing
    def do_search_bing(self, word):
        print('Searcing Bing...')
    ...
    # google
    def do_search_google(self, word):
        ...
        while self.counter <= self.limit and self.counter <= 1000:
            ...
            if self.search(self.results_google):
    ...
    def do_search_hunter(self, word):
        ...
        self.total_results_hunter = response[0].content.decode('UTF-8')
        result['result_hunter'] = self.total_results_hunter
        print('OK - Hunter!')
    #################################

    # yahoo
    def do_search_yahoo(self, word):
    ...
    # get delay : random float number 
    def getDelay(self) -> float:
        return random.randint(1, 3) - .5
    ############################

api.add_resource(SearchWord, '/search/<word>')
